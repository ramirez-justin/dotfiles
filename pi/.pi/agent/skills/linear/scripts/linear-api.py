...
def token() -> str:
    ...
    if not value:
        die("Missing LINEAR_API_KEY. Add it to ~/.pi/agent/env.local.zsh, preferably via 1Password injection.")
    return value
...
def main():
    ...
    s = sub.add_parser("viewer"); s.set_defaults(func=cmd_viewer)
    s = sub.add_parser("teams"); s.add_argument("--first", type=int, default=50); s.set_defaults(func=cmd_teams)
    s = sub.add_parser("assigned"); s.add_argument("--first", type=int, default=25); s.set_defaults(func=cmd_assigned)
    s = sub.add_parser("search"); s.add_argument("term"); s.add_argument("--first", type=int, default=25); s.set_defaults(func=cmd_search)
    s = sub.add_parser("get"); s.add_argument("issue", help="Issue key like ABC-123 or Linear issue UUID"); s.set_defaults(func=cmd_get)

    s = sub.add_parser("create")
    s.add_argument("--team-id", required=True); s.add_argument("--title", required=True); s.add_argument("--description")
    s.add_argument("--priority", type=int); s.add_argument("--assignee-id"); s.add_argument("--state-id"); s.add_argument("--yes", action="store_true")
    s.set_defaults(func=cmd_create)

    s = sub.add_parser("update")
    s.add_argument("issue"); s.add_argument("--title"); s.add_argument("--description"); s.add_argument("--priority", type=int)
    s.add_argument("--assignee-id"); s.add_argument("--state-id"); s.add_argument("--yes", action="store_true")
    s.set_defaults(func=cmd_update)

    s = sub.add_parser("comment")
    s.add_argument("issue"); s.add_argument("--body", required=True); s.add_argument("--yes", action="store_true")
